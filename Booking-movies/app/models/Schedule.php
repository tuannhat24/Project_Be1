...
class Schedule extends Database
{
    ...
    public function getScheduleByMovieTheaterTime($movie_id, $theater_id, $show_time, $price)
    {
        // Format the show_time for comparison
        $formatted_show_time = date('Y-m-d H:i:s', strtotime($show_time));

        $sql = "SELECT s.*, m.title as movie_title, t.name as theater_name
                FROM schedules s
                JOIN movies m ON s.movie_id = m.id
                JOIN theaters t ON s.theater_id = t.id
                WHERE s.movie_id = ? AND s.theater_id = ? AND s.show_time = ? AND s.price = ?";

        $stmt = self::$connection->prepare($sql);
        $stmt->bind_param("iisd", $movie_id, $theater_id, $formatted_show_time, $price);
